...
static MODEL: OnceLock<Mutex<Option<PotionModel>>> = OnceLock::new();
...
///
/// # Returns
/// Vector of i8 quantized embedding values
#[wasm_bindgen]
pub fn embed_quant_i8(sentence: String) -> Vec<i8> {
    let model = MODEL.get_or_init(|| Mutex::new(None)).lock().unwrap();
    model.as_ref().unwrap().embed_quant_i8(&sentence)
}
...
///
/// # Returns
/// Flattened vector of i8 quantized embedding values for all sentences
#[wasm_bindgen]
pub fn batch_embed_quant_i8(sentences: Vec<String>) -> Vec<i8> {
    let model = MODEL.get_or_init(|| Mutex::new(None)).lock().unwrap();

    let mut embeddings = Vec::with_capacity(sentences.len() * model.as_ref().unwrap().dim);

    for sentence in sentences {
        embeddings.extend(model.as_ref().unwrap().embed_quant_i8(&sentence));
    }
